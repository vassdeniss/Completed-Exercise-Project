...
namespace Eventures.WebApp.SeleniumTests
{
    public class SeleniumTestsUser : SeleniumTestsBase
    {
        ...
        [Test]
        public void Test_User_Register_Invalid()
        {
            // Arrange: go to the "Registration" page
            driver.Navigate().GoToUrl(this.baseUrl + "/Identity/Account/Register");

            // Locate fields and fill them in without username
            driver.FindElement(By.Id("Input_Email")).SendKeys($"{username}@mail.com");
            driver.FindElement(By.Id("Input_Password")).SendKeys(password);
            driver.FindElement(By.Id("Input_ConfirmPassword")).SendKeys(password);
            driver.FindElement(By.Id("Input_FirstName")).SendKeys("Pesho");
            ...
            // Assert page contains the text "The Username field is required."
            Assert.That(this.driver.PageSource.Contains("The Username field is required."));
        }
        ...
        [Test]
        public void Test_User_Login_Invalid()
        {
            // Arrange: go to the "Login" page
            driver.Navigate().GoToUrl(this.baseUrl + "/Identity/Account/Login");

            // Locate fields and fill them in with invalid credentials
            driver.FindElement(By.Id("Input_Username")).SendKeys("invalidUsername");
            driver.FindElement(By.Id("Input_Password")).SendKeys(password);
            ...
            // Assert page contains the text "Invalid login attempt."
            Assert.That(this.driver.PageSource.Contains("Invalid login attempt."));
        }
